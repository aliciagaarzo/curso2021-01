...
class CreateTicket(models.TransientModel):
    ...
    def create_ticket(self):
        ...
        if active_id and self._context.get('active_model') == 'helpdesk.ticket.tag':
            ...
            action = self.env.ref('helpdesk_aliciagarzo.helpdesk_ticket_action').read()[0]
            action['res_id'] = ticket.id
            action['views'] = [(self.env.ref('helpdesk_aliciagarzo.view_helpdesk_ticket_form').id, 'form')]
            return action
